...
export function sampleWalletResponse(): WalletAnalysisResponse {
  const now = Math.floor(Date.now() / 1000);
  const transactions: NormalizedTransaction[] = Array.from({ length: 18 }).map((_, i) => ({
    signature: `sample-${i}`,
    blockTime: now - i * 3600,
    type: i % 6 === 0 ? 'Swap' : i % 3 === 0 ? 'Token Transfer' : 'Transfer',
    source: `SourceWallet${i}`,
    destination: `DestWallet${i}`,
    amount: Math.round((Math.random() * 5 + 0.01) * 1e9),
    fee: 5000,
  ...
  const trustScore = {
    overall: Math.round(trustScoreValue),
    riskLevel: trustScoreValue < 40 ? (trustScoreValue < 20 ? 'high' : 'medium') : 'low',
    recommendations: buildRecommendations(Math.round(trustScoreValue), maliciousAnalysis, anomalyScore),
  } as const;

  return {
    address: 'SampleWalletAddress1111111111111111111111111',
    totalTransactions: transactions.length,
    totalVolume,
    transactions,
    anomalyScore,
